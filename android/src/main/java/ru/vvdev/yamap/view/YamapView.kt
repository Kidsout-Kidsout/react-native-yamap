...
open class YamapView(context: Context?) : MapView(context), UserLocationObjectListener,
    ...
    fun setBounds(bottomLeft: Point, topRight: Point, offset: Float, duration: Float, animation: Int) {
      val boundingBox = BoundingBox(bottomLeft, topRight)
      var cameraPosition = mapWindow.map.cameraPosition(Geometry.fromBoundingBox(boundingBox))
      cameraPosition = CameraPosition(
        cameraPosition.target,
        cameraPosition.zoom - offset,
      ...
      if (duration > 0) {
        val anim = if (animation == 0) Animation.Type.SMOOTH else Animation.Type.LINEAR
        mapWindow.map.move(cameraPosition, Animation(anim, duration), null)
      } else {
